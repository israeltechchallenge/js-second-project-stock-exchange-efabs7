class SearchForm {
   ...
   createDomElements =  () =>  {
       ...
       console.log('i have created the elements')
        }
       ...
       /* this.input.addEventListener('input', (e) => {
            e.preventDefault()
            debounce(this.getResults(), delayDebounce ) 
        })*/
    }
    ...
    async getResults () {
        try {
            showLoader()
            const response = await fetch (`${baseLink}search?query=${this.input.value}&limit=10&exchange=NASDAQ`)
      ...
      showResultList = (smbl, img, compName, changeP) => {
        ...
        if (percentageNum < 0) {
        newItem.innerHTML = `<img src=${img}></img>  ${compName}  (${smbl}) (${changeP.fontcolor("red")})` } 
     
        else {
             newItem.innerHTML = `<img src= ${img}></img>  ${compName}  (${smbl}) (${changeP.fontcolor("lightgreen")})`
        }
